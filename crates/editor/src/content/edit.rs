...
pub struct EditDelta {
    ...
    /// Wrapped in `Arc` to avoid expensive deep clones when the delta is propagated
    /// through the event system to multiple subscribers (render state, delay rendering, etc.).
...
impl EditDelta {
    ...
    pub fn layout_delta(
        ...
        // Unwrap the Arc if we're the sole owner; otherwise clone the inner vec.

...
- `complete_wait_action(conversation_id, tool_call_id, result)` sends `result` on the pending wait's channel. This is the single entry point used by both the watchdog fire and the inbound-resume path; the action's spawned future resolves with the received result and the action_model emits `FinishedAction` through its normal path.
- The executor subscribes to `BlocklistAIHistoryEvent::UpdatedConversationStatus` in its constructor. When a conversation it is waiting on transitions out of `WaitingForEvents`, the subscription calls `complete_wait_action(.., Completed)` so the action_model finishes the wait through the channel. This is the **sole** completion path for the inbound-supersede case; the transition is driven by whichever status writer fired first (typically `send_request_input` on the next outbound request — see §8.2).
- `cancel_execution(tool_call_id)` is invoked from the executor dispatch's cancel path. It drops the pending entry and bumps the generation counter so a still-pending watchdog timer no-ops on fire. The cancelled result (`WaitForEventsResult::Cancelled`) is produced by the cancel path's `cancelled_result()` machinery.
...
#### 8.2 Resume path: status transition completes the running wait
Two inbound messages can close the unresolved `WaitForEvents` tool call and resume the agent:
1. **Generic `Cancel` tool-call result (inbound supersede).** When new user input, an inbound message, or an inbound lifecycle event arrives on the waiting task, the server's existing pending-tool-call supersede mechanism appends a generic `Cancel` tool-call-result referencing the unresolved `WaitForEvents` id (server TECH §1.1).
...
In both cases, the inbound message is just transcript data — `apply_client_actions` appends it to the conversation transcript with no special handling. The status flip that completes the wait is driven by the **outbound** side, not by inbound message-shape inspection. Specifically:
- For both subpaths, the resume signal is preceded by an outbound request from the client (a user-typed message in the user-input case, an `inject_pending_events_for_request` drain in the orchestration-event case, or the auto-follow-up triggered after the watchdog fires). Every outbound request flows through `BlocklistAIController::send_request_input`, which calls `BlocklistAIHistoryModel::update_conversation_status(InProgress)` synchronously.
- That status flip emits `BlocklistAIHistoryEvent::UpdatedConversationStatus`. The `WaitForEventsExecutor`'s subscription consumes the event, sees the conversation is no longer `WaitingForEvents`, and calls `complete_wait_action(.., Completed)`. The action's spawned future resolves with `Completed`, the action_model emits `FinishedAction`, and the controller's auto-follow-up subscriber observes the in-flight stream and returns without sending a duplicate request.
For the **cancel-during-wait** case (Ctrl+C), the executor's `cancel_execution` path drops the `pending` entry directly and the action_model emits `FinishedAction(Cancelled)`; the executor's `UpdatedConversationStatus` subscription is a no-op because `pending` is already empty by the time the `Cancelled` status flip fires.
...
9. **Resume by inbound supersede.** New user input, an inbound message, or an inbound lifecycle event reaches the waiting task. In every subpath, the resume is driven by an outbound request from the client (the user's message submission, an `inject_pending_events_for_request` drain triggered by `EventsReady`, etc.). That outbound request flows through `send_request_input`, which flips status to `InProgress`. The `WaitForEventsExecutor`'s `UpdatedConversationStatus` subscription completes the running wait via `complete_wait_action(.., Completed)`, the action_model emits `FinishedAction`, and the controller's auto-follow-up subscriber sees the in-flight stream and bails. The server-synthesized `Cancel` (or echoed `WaitForEventsResult`) arrives in the response stream that follows and is appended to the transcript as ordinary message data by `apply_client_action(AddMessagesToTask)`.
10. **Resume by watchdog timeout.** If no inbound input arrives before the watchdog fires, the executor's timer callback verifies the generation counter still matches and calls `complete_wait_action(.., Completed)` directly. The auto-follow-up subscriber sends a follow-up request whose input includes the empty `WaitForEventsResult` produced by the action's result conversion. The server echoes the result through the next stream; the agent's next turn observes the empty timeout result and decides how to proceed (commonly `finish_task`, but the agent may also re-yield, ask the user, or take other action). The run is **not** auto-cancelled on timeout; the agent owns the decision.
